...
export default function Sidebar() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('bmc_theme') as 'dark' | 'light';
    if (saved) {
      setTheme(saved);
  ...
  const toggleTheme = () => {
    ...
    localStorage.setItem('bmc_theme', next);
  };
      ...
      <aside className={`sidebar ${isOpen ? 'open' : ''}`}>
        ...
        <div style={{ padding: '0.5rem 1rem', marginTop: 'auto' }}>
          ...
          >
            ...
            <span className="sidebar-link-text">{theme === 'dark' ? 'Modo Claro' : 'Modo Escuro'}</span>
          </button>
        </div>
