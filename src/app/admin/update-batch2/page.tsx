...
export default function UpdateBatch2Page() {
  ...
  const handleRun = async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/admin/update-batch2', { method: 'POST' });
  ...
  return (
    <div className="detail-page" style={{ maxWidth: '800px', margin: '2rem auto' }}>
      <h1 style={{ fontSize: '1.5rem', fontWeight: 800, marginBottom: '0.5rem', color: 'var(--text-primary)' }}>
        Atualizar Lote 2 — Função e Empresa
      </h1>
      <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem', marginBottom: '1.5rem' }}>
        ~230 clientes: atualiza FUNÇÃO e EMPRESA (somente campos vazios).
      </p>
      ...
      >
        {loading ? '⏳ Atualizando...' : '🚀 Executar Atualização Lote 2'}
      </button>
