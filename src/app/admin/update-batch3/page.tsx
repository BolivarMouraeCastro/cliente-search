...
export default function UpdateBatch3Page() {
  ...
  const handleRun = async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/admin/update-batch3', { method: 'POST' });
  ...
  return (
    <div className="detail-page" style={{ maxWidth: '800px', margin: '2rem auto' }}>
      <h1 style={{ fontSize: '1.5rem', fontWeight: 800, marginBottom: '0.5rem', color: 'var(--text-primary)' }}>
        Atualizar Lote 3 (Final) — Função e Empresa
      </h1>
      <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem', marginBottom: '1.5rem' }}>
        ~105 clientes: atualiza FUNÇÃO e EMPRESA (somente campos vazios).
      </p>
      ...
      <button
        ...
        style={{
          ...
          background: loading ? 'rgba(255,255,255,0.05)' : 'linear-gradient(135deg, #10b981, #14b8a6)',
          color: '#fff', fontWeight: 700, fontSize: '0.9rem', cursor: loading ? 'not-allowed' : 'pointer',
          marginBottom: '1.5rem',
        }}
      >
        {loading ? '⏳ Atualizando...' : '🚀 Executar Atualização Final (Lote 3)'}
      </button>
      ...
      {result && (
        <div style={{
          ...
          <div style={{ display: 'flex', gap: '2rem', marginBottom: '1rem', fontSize: '0.85rem', flexWrap: 'wrap' }}>
